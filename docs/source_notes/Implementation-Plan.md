...
Phần slide và report cũng cần phân công sớm, không để dồn về cuối:
- member-1: viết phần Solution 1 cho report và chuẩn bị nội dung slide của Solution 1.
- member-2: viết phần Solution 2 cho report và chuẩn bị nội dung slide của Solution 2.
- member-3: viết phần dataset/data preprocessing cho report, cung cấp bảng/tóm tắt dữ liệu cho slide, và là người ghép final slide deck.
- member-4: viết phần validation/evaluation cho report, tổng hợp nội dung so sánh kết quả giữa hai solution, và là người ghép final report.
...
5. Sau khi có output đánh giá sơ bộ, mỗi thành viên bàn giao phần slide thô cho member-3 và phần report thô cho member-4 để ghép bản chung.
...
### 3.7. Phân công slide và report

#### Phân công report

- member-1: viết phần `Solution 1`, gồm pipeline, input/output, điểm mạnh/yếu và rủi ro triển khai.
- member-2: viết phần `Solution 2`, gồm pipeline, input/output, logic parse/re-rank và điểm mạnh/yếu.
- member-3: viết phần `Dataset`, `Data preprocessing`, `Schema dữ liệu`.
- member-4: viết phần `Validation set`, `Evaluation protocol`, `So sánh kết quả`, và ghép final report.

#### Phân công slide

- member-1: chuẩn bị 1-2 slide cho `Solution 1`.
- member-2: chuẩn bị 1-2 slide cho `Solution 2`.
- member-3: chuẩn bị 1 slide cho `Dataset/Data overview` và ghép final slide deck.
- member-4: chuẩn bị 1-2 slide cho `Validation/Evaluation/Comparison`.

#### Nguyên tắc ghép bản chung

- Mỗi thành viên phải bàn giao phần nội dung thô trước khi ghép format cuối.
- Member-3 là người ghép bản final của slide để đảm bảo format và flow trình bày thống nhất.
- Member-4 là người ghép bản final của report để đảm bảo thuật ngữ, cấu trúc viết và nội dung đánh giá thống nhất.
- Slide và report phải dùng cùng thuật ngữ với output contract và validation set.
- Không tự đổi tên field, tên metric hoặc tên solution trong slide/report nếu chưa thống nhất cả nhóm.
...
- Chuẩn bị nội dung report, slide và tài liệu xin ý kiến giảng viên theo phân công đã chốt.
...
4. Bộ nội dung report/slide bản nháp từ từng thành viên.
5. Biên bản demo: tiến độ, rủi ro, hướng ưu tiên và kế hoạch tiếp theo.
...
- `10/07 - 12/07`: bàn giao bảng số liệu, mô tả dataset ngắn và nội dung dataset cho report/slide.
- `12/07`: ghép final slide draft từ toàn bộ nội dung slide thô của cả nhóm.
...
- `11/07 - 12/07`: bàn giao phần validation/evaluation cho report và slide comparison.
- `12/07`: ghép final report draft từ toàn bộ nội dung report thô của cả nhóm.
...
- `11/07 - 12/07`: bàn giao `report thô` của Solution 1 cho member-4 và `slide thô` của Solution 1 cho member-3.
...
- `10/07`: xuất kết quả trên validation set chung cho member-4.
- `11/07 - 12/07`: sửa lỗi theo phản hồi từ validation và chuẩn bị demo.
- `11/07 - 12/07`: bàn giao `report thô` của Solution 2 cho member-4 và `slide thô` của Solution 2 cho member-3.
...
| Report thô của Solution 1 | member-1 | 12/07/2026 | member-4 |
| Slide thô của Solution 1 | member-1 | 12/07/2026 | member-3 |
| Report thô của Solution 2 | member-2 | 12/07/2026 | member-4 |
| Slide thô của Solution 2 | member-2 | 12/07/2026 | member-3 |
| Report thô phần dataset | member-3 | 12/07/2026 | member-4 |
| Slide thô phần dataset/data overview | member-3 | 12/07/2026 | member-3 |
| Report thô phần validation/comparison | member-4 | 12/07/2026 | cả nhóm |
| Slide thô phần validation/comparison | member-4 | 12/07/2026 | member-3 |
| Final slide draft | member-3 | 12/07/2026 | cả nhóm |
| Final report draft | member-4 | 12/07/2026 | cả nhóm |
...
| Tuần 2 | M1.6 | 12/07/2026 | Bàn giao `report thô` và `slide thô` phần dataset, đồng thời ghép final slide | Report thô dataset + slide thô dataset/data overview + final slide draft |
...
| Tuần 2 | M2.8 | 12/07/2026 | Bàn giao `report thô` và `slide thô` của Solution 1 | Report thô Solution 1 + slide thô Solution 1 + output mẫu |
...
| Tuần 2 | M3.8 | 12/07/2026 | Bàn giao `report thô` phần validation/comparison và ghép final report | Report thô validation/evaluation/comparison + final report draft |
...
| Tuần 2 | M4.8 | 12/07/2026 | Bàn giao `report thô` và `slide thô` của Solution 2 | Report thô Solution 2 + slide thô Solution 2 + output mẫu |
...
| Tuần 2 | 06/07 – 12/07 | Demo và chốt hướng | Bản triển khai/demonstration + kết quả đánh giá sơ bộ + bảng so sánh output chung + các phần report thô + final report draft + các phần slide thô + final slide draft + biên bản demo | Planned |

## 8. Bảng theo dõi tiến độ theo thành viên

| Thành viên | Tuần 1 | Tuần 2 | Ghi chú |
|---|---|---|---|
| member-1 | Thiết kế Solution 1 theo output contract chung | Triển khai Solution 1 + bàn giao report thô Solution 1 + slide thô Solution 1 | Solution 1 |
| member-2 | Thiết kế Solution 2 theo output contract chung | Triển khai Solution 2 + bàn giao report thô Solution 2 + slide thô Solution 2 | Solution 2 |
| member-3 | Rà soát schema/cột bắt buộc của dataset chung | Hỗ trợ dữ liệu + bàn giao report thô dataset + ghép final slide | Dataset + slide assembler |
| member-4 | Dựng validation set bản 1 + chốt output contract + tiêu chí đánh giá | Chạy validation + bàn giao report thô validation/comparison + ghép final report | Validation + report assembler |
