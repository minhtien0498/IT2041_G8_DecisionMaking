...
Quy ước hiện tại:
- baseline rule-based cũ đã bị loại, chỉ giữ làm historical reference
...
1. Với mỗi persona, chạy `Solution 2` và `Solution 1`.
...
Dùng làm **evaluation phụ** để chứng minh solution nào có trải nghiệm tốt hơn với người dùng thật.
...
2. Chạy cả `Solution 2` và `Solution 1`.
3. Kiểm tra:
...
| Metric | Solution 2 kỳ vọng | Solution 1 kỳ vọng |
|---|---|---|
| Latency | Cao hơn | Thấp hơn / ổn định hơn |
| API cost | Cao hơn | Thấp hơn nếu chạy offline |
| Implementation complexity | Cao | Trung bình-cao |
| Interpretability | Trung bình-cao | Cao |
| Personalization | Cao | Trung bình-cao |
| Scalability | Phụ thuộc API/cache | Cao nếu chuẩn hóa ma trận quyết định |

### Khi nào dùng

Dùng để giải thích vì sao nhóm giữ cả 2 solution chính:

- `Solution 2` tối ưu về cá nhân hóa và xử lý nhu cầu tự nhiên.
- `Solution 1` tối ưu về reasoning có kiểm soát, tool-use và grounding guardrail.

## 8. So Sánh Solution 2 và Solution 1

| Tiêu chí | Solution 2 | Solution 1 |
|---|---|---|
| Constraint satisfaction | Cao nếu parser + rerank đúng | Cao nếu hard filter và guardrail đúng |
| Top-K relevance | Tốt hơn khi parser/enrichment đo đúng nhu cầu | Tốt hơn khi LLM reasoner gọi tool và chấm đúng trade-off |
| Explainability | Cao nếu explanation bám đúng score và enriched data | Cao nếu explanation bám đúng Top 5 và dữ liệu thật |
| Flexibility | Cao | Cao |
| Latency | Trung bình-cao | Cao hơn do nhiều lượt LLM/tool |
| API cost | Cao hơn nếu enrich động | Cao hơn nếu dùng LLM/tool nhiều lượt |
| Risk | Parser sai, POI thiếu | LLM reasoning sai, explanation dài/sai, latency cao |
| Phù hợp final | Cao | Cao |
...
- Both main solutions run on the same property subset.
