"""
Phiên bản ghi chú cho member-3:
Trích xuất 100 căn "Nhà riêng" sạch từ data_public.csv
...
def normalize_text(text):
    """Chuẩn hóa chuỗi để dò tên quận không dấu."""
...
def is_valid_row(row):
    """Giữ lại các dòng đủ sạch cho dataset 100 căn."""
...
def detect_district(row):
    """Nhận diện quận từ Location + Title."""
...
def extract_ward(location):
    """Tách phường từ chuỗi Location nếu có."""
...
def clean_row(row, district_name, prefix, idx):
    """Map dữ liệu CSV thô sang schema JSON chung của nhóm."""
...
def main():
    ...
    for district_name, quota in quotas.items():
        ...
        if len(district_rows) < quota:
            raise SystemExit(
                f"Không đủ listing sạch cho {district_name}: có {len(district_rows)}, cần {quota}"
            )
